...
@login_required
def csv_upload_view(requests):
    print('file sending')
    if requests.method == 'Post':
        csv_file_name = requests.FILES.get('file').name
        csv_file = requests.FILES.get('file')
        ...
        if created:
            ...
            with open(obj.csv_file.path, 'r') as f:
                ...
                for row in reader:
                    ...
                    try: 
                        product_obj = Product.objects.get(name_iexact=product)
                    except Product.DoesNotExist:
                        product_obj = None
                    
                    if product_obj is not None:
                        customer_obj, _ = Customer.objects.get_or_create(name=customer)
                        salesman_obj = Profile.objects.get(user=requests.user)
